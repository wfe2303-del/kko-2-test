...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if(action === 'health'){
      res.statusCode = 200;
      res.end(JSON.stringify(await storage.health()));
      return;
    }
    ...
    res.end(JSON.stringify({
      error: error && error.message ? error.message : 'Stored execution request failed.'
    }));
  }
};
...
async function handleListPending(req, res, user){
  ...
  response = await storage.listPending(sheetTitle);
...
async function handleSyncRun(req, res, user){
  ...
  response = await storage.syncRun(payload);
  res.statusCode = 200;
...
async function handleApplyManualAction(req, res, user){
  ...
  payload.actorEmail = user.username;
  payload.actorName = user.displayName;
  response = await storage.applyManualAction(payload);
  res.statusCode = 200;
...
async function handleApplyManualActionsBatch(req, res, user){
  ...
  payload.actorEmail = user.username;
  payload.actorName = user.displayName;
  response = await storage.applyManualActionsBatch(payload);
  res.statusCode = 200;
  res.end(JSON.stringify(response || { enabled: true, items: [], manualRules: [] }));
}
...
async function handleStoredSheetData(req, res, user){
  ...
  response = await storage.getStoredSheetData(sheetTitle);
  res.statusCode = 200;
