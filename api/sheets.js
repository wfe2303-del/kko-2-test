...
var googleSheets = require('./_lib/google-sheets');

module.exports = async function handler(req, res) {
  setJsonHeaders(res);

  var session = auth.getSession(req);
  var spreadsheetId = googleSheets.getAttendanceSpreadsheetId();
  ...
  if(!googleSheets.isConfigured()){
    res.statusCode = 503;
    res.end(JSON.stringify({ error: 'Google Sheets backend is not configured. Set KAKAO_CHECK_SPREADSHEET_ID and Google service account credentials.' }));
    return;
  }
...
async function handleGet(req, res, user, spreadsheetId){
  ...
  if(action === 'listSheetTitles'){
    var titles = auth.filterAllowedSheets(user, await googleSheets.listSheetTitles(spreadsheetId));
    res.statusCode = 200;
  ...
  if(action === 'loadRosterRows'){
    ...
    roster = await googleSheets.loadRosterRows({
...
async function handlePost(req, res, user, spreadsheetId){
  ...
  payload = await googleSheets.writeUpdates({
    spreadsheetId: spreadsheetId,
    updates: updates
  });
