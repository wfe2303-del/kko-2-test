(function(){
  ...
  async function loadSheetTitles(){
    ...
    try {
      ...
      if(!state.sheetTitles.length){
        ui.setAppNotice('불러올 수 있는 시트가 없습니다. 계정 권한 또는 Google 서비스 계정의 스프레드시트 접근 권한을 확인해 주세요.', 'warn');
      }
    } catch (error) {
      state.sheetTitles = [];
  ...
  async function runPanel(panelId){
    ...
    try {
      ...
      if(!rosterResponse.rows.length){
        throw new Error('선택한 시트에서 N열이 수강생인 명단을 찾지 못했습니다. Google 서비스 계정 권한과 N열 값을 확인해 주세요.');
      }
  ...
  function toUserMessage(error){
    ...
    if(message.indexOf('Supabase is not configured.') >= 0){
      return 'Supabase 환경변수가 설정되지 않았습니다. Vercel에 KAKAO_CHECK_SUPABASE_URL과 KAKAO_CHECK_SUPABASE_SERVICE_ROLE_KEY를 넣어 주세요.';
    }
    if(message.indexOf('Supabase query failed') >= 0){
      return 'Supabase 저장소 요청이 실패했습니다. Supabase 테이블 생성과 service_role 키 설정을 확인해 주세요.';
    }
    if(message.indexOf('Google Sheets backend is not configured.') >= 0){
      return 'Google Sheets 환경변수가 설정되지 않았습니다. 서비스 계정 설정과 시트 ID를 확인해 주세요.';
    }
    if(message.indexOf('Failed to load Google service account credentials.') >= 0){
      return 'Google 서비스 계정 키를 읽지 못했습니다. JSON 값 또는 파일 경로를 다시 확인해 주세요.';
    }
